...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_execute_debitTransaction` (IN `varDate` TEXT, IN `varAccountNo` TEXT, IN `varVoucher` TEXT, IN `varReason` TEXT, IN `varPreviousBalance` TEXT, IN `varEntryType` TEXT, IN `varAmount` TEXT, IN `varBalance` TEXT, IN `varEntryBy` TEXT)  BEGIN
DECLARE pAmt LONGTEXT;

END$$

CREATE DEFINER=`root`@`192.168.2.3` PROCEDURE `sp_insert_Invoice` (IN `varDateTime` TEXT, IN `varSubTotal` TEXT, IN `varDiscount` TEXT, IN `varTax` TEXT, IN `varOther` TEXT, IN `varTotal` TEXT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_FeatureAdmin` (IN `spintEmpId` TEXT)  BEGIN
SELECT DISTINCT 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_RoleFeaturesMap` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_SubMenuFromFeatureAdmin` (IN `varEmpId` TEXT, IN `spParentMenuId` TEXT)  BEGIN
SELECT DISTINCT 
                         tblskpersonnel.varEmpName, featuresmaster.varFeatureName, featuresmaster.PageName, featuresmaster.isSubMenu, featuresmaster.CreatedDate, 
                         featuresmaster.varIcon, featuresmaster.SortOrder, featuresmaster.IsActive, featuresmaster.FeatureId, featuresmaster.ParentId
FROM            tblskpersonnel INNER JOIN
...
(1, 'SKSI5201913', '2016-07-02', '70000', 'NA', 'NA', 'NA', '70000', '', '', '', '', '', '', ''),
(2, 'SKSI8821814', '2016-07-02', '341', 'NA', 'NA', 'NA', '341', '', '', '', '', '', '', '');
...
(1, 'SKSI5201913', 'df', 'NA', '70000', '', '', '', '', '', '', ''),
(2, 'SKSI8821814', 'dgdff', 'NA', '341', '', '', '', '', '', '', '');
...
(1, 2, 'SKS838477', 'SKSI5201913', '0', '70000', 'Pending', '0', 'Started', 'NA', 'Started', '', '', '', ''),
(2, 1, 'SKSV9346489', 'SKSI8821814', '0', '341', 'Pending', '0', 'Started', 'NA', 'Started', '', '', '', ''),
(3, 1, 'SKSV5106484', 'SKSI8821814', '0', '341', 'Pending', '0', 'Started', 'NA', 'Started', '', '', '', ''),
(4, 1, 'SKSV2274028', 'SKSI8821814', '0', '341', 'Pending', '0', 'Started', 'NA', 'Started', '', '', '', '');
...
--
-- Dumping data for table `tblsksupport`
--

INSERT INTO `tblsksupport` (`intId`, `varPersonneId`, `intPersonelRole`, `varDate`, `varName`, `varUsername`, `varMobile`, `varEmail`, `varSubject`, `varDescription`, `varStatus`, `varRemark`, `intIsActive`, `ex1`, `ex2`, `ex3`, `ex4`, `ex5`) VALUES
(1, '', 0, '2016-07-01', 'Bhushan Savale', 'jyotipatil', '9561421489', 'savalebd@gmail.com', 'login not work', 'login not work', 'Resolved', '', 1, '', '', '', '', ''),
(2, '', 0, '2016-07-01', 'Bhushan Savale', 'fgfg', '9561421489', 'savalebd@gmail.com', 'login not work', 'CssClass="comment more"CssClass="comment more"', 'Pending', '', 1, '', '', '', '', '');
